...
const TrackerLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://localhost:5000/api/tracker/login", {
        method: "POST",
        headers: {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/tracker/signup", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    >
      ...
      >
        <h2 className="text-2xl font-bold mb-6 text-center text-gray-800">
          {isSignup ? "Tracker Sign Up" : "Tracker Login"}
        </h2>
